...
(function($) {
	    ...
	    init : function(options) {
		    ...
		    return this
		        .each(function() {
			        ...
			        var loadPicker = function() {
				        ...
				        if (calendar.length == 0) {
                        	...
                        	<div class="calendar_header">'
					                    + '<div class="calendar_nav calendar_p_y">&lt;&lt;</div>\
				        ...
				        var setDayData = function(date) {
					        ...
					        for (var i = 0; i < yearMonthDay.length; i++) {
						        ...
						        if (date.getMonth() == temp.dateVal.getMonth()
						            && date.getDate() == temp.dateVal.getDate()) {
							        style += ' selected';
						        }
						        style += checkRestrictDate(temp.dateVal);
						        var data = ' data-options="' + formatDays(temp.dateVal) + '"';
						        html += '<li class="' + style + '"' + data + '>' + formatDays(temp.dateVal).substr(8)
						            + '</li>';
					        }
				        ...
				        setWeekData = function(date) {
					        ...
					        for (var i = 0; i < yearWeeks.length; i++) {
						        ...
						        var data = ' data-options="' + year + '-' + obj.index + ',' + formatDays(obj.monday)
						            + ',' + formatDays(obj.sunday) + '"';
						        html += '<li class="' + style + '"' + data + '><span>第' + obj.index
						            + '周</span>\
                                <i>'
						            + formatDays(obj.monday).substr(5) + '至' + formatDays(obj.sunday).substr(5)
						            + '</i></li>';
					        }
					        calendar_ul.append(html);
					        // 绑定点击事件
					        calendar_ul.find('li').click(function() {
						        ...
						        var monday = data.split(',')[1], sunday = data.split(',')[2];
				        ...
				        setMonthData = function(date) {
					        ...
					        for (var i = 0; i < yearMonth.length; i++) {
						        ...
						        var data = ' data-options="' + year + '-' + fixZore(i + 1) + ',' + formatDays(fDay)
						            + ',' + formatDays(lDay) + '"';
			        ...
			        // 对象
			        var target = $(this), targetName = target.attr('id');
			        // 不要时间部分，设置为0
	...
	getMonDayAndSunDay = function(weekth) {
		...
		var y = weekth.substr(0, 4), w = weekth.substr(4);
		// 第一天
