            ...
            return this.each(function() {
                ...
                var loadPicker = function() {
                    ...
                    if (calendar.length == 0) {
                        $(document.body).append('<div id="calendar_' + targetName + '" class="calendar">\
                    ...
                    calendar.css({
                        left: targetOffsetLeft - 3,
                        top: targetOffsetTop + target.outerHeight() + 1,
                    ...
                    var setDayData = function(date) {
                            ...
                            // 绑定点击事件
                            calendar_ul.find('li.day').click(function() {
                                $(this).siblings('li').removeClass('selected').end().addClass('selected');
                                var data = $(this).attr('data-options');
                                var dataVal = data.split('-');
                                settings.selectDay.setFullYear(dataVal[0], parseInt(dataVal[1], 10) - 1, dataVal[2]);
                                dataVal = settings.fmtDay.replace(/(y+)/, dataVal[0]).replace(/(M+)/, dataVal[1]).replace(/(d+)/, dataVal[2]);
                                target.val(dataVal);
                    ...
                    switch (settings.type) {
                        case 'day':
                            current.setFullYear(settings.selectDay.getFullYear(), settings.selectDay.getMonth(), settings.selectDay.getDate());
                            break;
                        case 'week':
                            current.setFullYear(settings.selectWeek.getFullYear(), settings.selectWeek.getMonth(), settings.selectWeek.getDate());
                            break;
                        case 'month':
                            current.setFullYear(settings.selectMonth.getFullYear(), settings.selectMonth.getMonth(), settings.selectMonth.getDate());
                            break;
                    }
            ...
            if (options) {
                ...
                switch (settings.type) {
                    ...
                    case 'day':
                        ...
                        temp = dayVal.split('-');
                        dateVal = settings.fmtDay.replace(/(y+)/, temp[0]).replace(/(M+)/, temp[1]).replace(/(d+)/, temp[2]);
                        $(this).parent().find('i').remove();
                        $(this).width(90).css({
                            fontWeight: 'normal'
                        });
